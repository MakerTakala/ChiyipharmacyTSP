...
float minlength(int startnode, int visitedplace){
    float MIN = FLT_MAX;
    for(int i = 1; i < N; i++){
        if( (i !=startnode) && (visitedplace>>i & 1) ){
            MIN = min(MIN , DataBase[startnode][i] + LengthDataBase[i][visitedplace - (1<<i)]);
        }
    }
    cout<<startnode<<" "<<visitedplace<<" "<<MIN<<endl;
    return MIN;
}
...
int main()
{
    UsedNumber[0] = true;
    for(int i = 1; i < N; i++){
        LengthDataBase[i][ (1<<0) + (1<<i) ] = DataBase[i][0];
    }
    ...
    float ANS = FLT_MAX;
        int n = (1<<N) - 1;
    for(int i = 1; i < N; i++){
        ANS = min(ANS, DataBase[i][0] + LengthDataBase[i][ n - (1<<i)] );
    }
    ...
    for(int i = 1; i < N; i++){
        int MIN = FLT_MAX, x;
        for(int j = 1; j < N; j++){
            if( ((n>>j)  & 1) && MIN >= LengthDataBase[j][n - (1<<j)]){
