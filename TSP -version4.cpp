...
void slove(int startnode, int quantity){
    if(ThroughPlaceNumber.size() == quantity){
        int X10VisitedPlace = 0;
        for(int i = 0; i < quantity; i++){
            X10VisitedPlace += 1<<ThroughPlaceNumber.top();
            ThroughPlaceNumber.pop();
        }
        LengthDataBase[startnode][X10VisitedPlace+1] = minlength(startnode, X10VisitedPlace+1);
    }
...
int main()
{
    ...
    for(int i = 1; i < N; i++){
        int MIN = FLT_MAX, j;
        for(int i = 1; i < N; i++){
            if( (n & 1<<i) && MIN >= LengthDataBase[i][n - 1<<i]){
                MIN = LengthDataBase[i][n - 1<<i];
                j = i;
